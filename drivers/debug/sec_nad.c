...
static void do_qmvs(int smd_nad)
{
	...
	switch(smd_nad) {
		case SMD_QMVS:
			...
			NAD_PRINT("SMD_QMVS, nad_test_mode : %d", nad_test_mode);
			break;
		case NOT_SMD_QMVS:
			argv[1] = NOT_SMD_QMVS_LOGPATH;
			NAD_PRINT("NOT_SMD_QMVS, nad_test_mode : %d", nad_test_mode);
			break;
		default :
			NAD_PRINT("Invalid smd_nad value, nad_test_mode : %d", nad_test_mode);
			break;
	}
